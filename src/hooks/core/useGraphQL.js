...
export const useQuery = (query, variables = {}, options = {}) => {
    ...
    const fetch = useCallback(async (fetchVariables) => {
        ...
        try {
            ...
            const transformedData = transformRef.current ? transformRef.current(result) : result;
 ...
 */
export const useMutationQuery = (mutationOrOptions, optionsArg = {}) => {
    // Detect which pattern is being used
    const isOptionsFirst = typeof mutationOrOptions === 'object' && mutationOrOptions !== null;
    const mutation = isOptionsFirst ? null : mutationOrOptions;
    const options = isOptionsFirst ? mutationOrOptions : optionsArg;
    ...
    const execute = useCallback(async (dynamicMutation, variables = {}) => {
        setLoading(true);
        setError(null);
