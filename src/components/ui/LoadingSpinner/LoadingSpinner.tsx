 ...
 */
export default function LoadingSpinner({ size = "medium", fullScreen = false }: LoadingSpinnerProps): React.ReactElement {
    const sizeClass = size === "large" ? "spinner-large" : "";

    const content = <div className={`spinner ${sizeClass}`}></div>;

    if (fullScreen) {
        return <div className="loading-container loading-container-full">{content}</div>;
    }
